module Darts
  ...
  def self.darts_needed(score, double_out: true)
    score = score.to_i
    return non_double_out_darts_needed(score) unless double_out
  ...
  def self.possible_checkout_darts?(score, darts, double_out: true)
    needed = darts_needed(score, double_out: double_out)
    !needed.nil? && darts.to_i >= needed && darts.to_i <= 3
  end

  def self.non_double_out_darts_needed(score)
    return nil if score <= 0 || score > 180
